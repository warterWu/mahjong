...
import com.mahjong.server.vo.CPG;
...
public class MahjongLogic {
	 ...
	 * @return
	 */
	public CPG isCPG(byte value, byte[] playerx, boolean isNext) {
	...
	public static void main(String[] args) {
		...
		// byte[] b=new byte[]{2, 2, 2, 2, 4, 4, 4, 4, 6, 6, 6, 6, 8, 8, 8, 8,
		// 10, 10};// 18罗汉 SUCCESS
		// byte[] b=new byte[]{1, 1, 2, 2, 3, 3, 5, 5, 7, 7, 9, 9, 4, 4};
		long start = System.currentTimeMillis();
		new MahjongLogic().shuffleAndDeal();
	...
	public void logic() {
		/***
		 * server.strart(); init Hall if(玩家连接服务器){ if(isFirst){ 记录玩家信息 }else{
		 * 获得玩家信息 } if(玩家进入大厅){ 获取ROOM列表 } if(进入ROOM){ 初始化ROOM，遍历状态，开是否开局 }
		 * if(如果都处于准备状态，开局){ 洗牌，发牌，走牌 if(有人hu){ 结束，算分。 } } }else{
		 * 
		 * }
		 */
		//
